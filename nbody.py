...
def advance(dt, n, bodies=SYSTEM, pairs=PAIRS):
    fieldnames = ['name of the body', 'position x', 'position y', 'position z']
    with open('position.csv', 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(fieldnames)
    for i in range(n):
        for name,([x,y,z],a,b) in BODIES.items():
            with open('position.csv', 'a', encoding='UTF8', newline='') as f:
                writer = csv.writer(f)
                data = '{k},{x},{y},{z}'.format(k=name, x=x, y=y, z=z)
                print(data)
...
def main(n, ref="sun"):
    ...
    advance(0.01, n)
    #report_energy()
    '''f = open("nbody_result.csv", "a")
    f.writelines(BODIES.values())
    f.close()'''
    #f = open()
